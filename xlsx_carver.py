...
def is_valid_xlsx(data: bytes, start_idx: int, end_idx: int) -> bool:
    """
    Strictly verifies that this ZIP archive is specifically an XLSX file.
    XLSX files must contain 'xl/workbook.xml'.
    We also check for '[Content_Types].xml'.
    """
    archive_slice = data[start_idx:end_idx].lower()
    if b'xl/workbook.xml' in archive_slice and b'[content_types].xml' in archive_slice:
        return True
    return False
...
def main():
    ...
    print("3. Starting Strict XLSX Carver Scan...")
            ...
            while idx < len(chunk) - 4:
                ...
                if end_of_file != -1:
                    ...
                    if len(extracted) > 1000:
                        if is_valid_xlsx(chunk, header_idx, end_of_file):
                            file_count += 1
                            fname = f"recovered_{file_count}.xlsx"
    ...
    print(f"\n[*] Scan completely Done. Recovered XLSX files: {file_count}")
